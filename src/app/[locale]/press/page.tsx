...
export async function generateMetadata({ params }: { params: { locale: string } }) {
  const t = await getTranslations({ locale: params.locale, namespace: 'pressPage' });
  ...
  return (
    <div className="container-content pb-24 pt-32">
      ...
      <section className="mt-16 border-t border-line pt-12">
        <h2 className="label-mono">{t('contactH')}</h2>
        <p className="mt-6 text-lg text-ink-dim">{t('contactP')}</p>
      </section>
    </div>
  );
}
